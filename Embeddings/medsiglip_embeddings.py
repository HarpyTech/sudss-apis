...
class Embedder:
    def __init__(self, model_name: str, device: torch.device, max_text_tokens: int = 128):
        ...
        # Will be set at runtime from first image/text embedding if available
    ...
    def embed_images(self, pil_images: List[Image.Image]) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            if self.embedding_dim is None:
                self.embedding_dim = emb_np.shape[1]
    ...
    def embed_texts(self, texts: List[str]) -> np.ndarray:
        ...
        This method is defensive: it handles models that expose `get_text_features` or
        return `text_embeds` / `pooler_output` / `last_hidden_state`. If the model
        returns an unexpected object, the function will fall back to a zero vector of
        the proper embedding dimension (if known) to preserve alignment with image
        embeddings.
        """
        if len(texts) == 0:
        ...
        with torch.no_grad():
            try:
                ...
                if text_feats is None:
                    raise RuntimeError("Model did not return identifiable text embeddings")
                ...
                # Defensive fallback: return zero vectors of appropriate dim to keep alignment
                print(f"Warning in embed_texts(): {e}")
